    ...
    onLoad: function () {
        ...
        this.node.on(cc.Node.EventType.TOUCH_START, (event) => {
            ...
            for (let i in this.controlPointList) {
                ...
                let dis = point.position.sub(this.node.parent.convertToNodeSpaceAR(event.getLocation())).len();
                if (dis < 10) {//触摸点的距离与控制点的距离小于10个像素,则算做相同点
                    touchPoint = point;
                }
            }
        });
        ...
        this.node.on(cc.Node.EventType.TOUCH_MOVE, (event) => {
            if (touchPoint) {
                touchPoint.position = this.node.parent.convertToNodeSpaceAR(event.getLocation());
            }
        });

        this.node.on(cc.Node.EventType.TOUCH_END, (event) => {
            if (touchPoint === undefined) {
                //触摸开始点没有在控制列表中,说明为新的控制点,将控制点添加到控制点列表中
